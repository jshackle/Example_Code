...
periodicities = Array{Float64}(undef, length(gamma_range));

for ii in 1:length(gamma_range)
#p = [I, k, M, alpha, gamma]
p = [4*10^(-6), 0.05, 0.1, 0.001, gamma_range[ii]];
#u0 = [A0, P0]
u0 = [2, log(10^(-3))];
...
periodicities[ii] = mean(p[(length(p)-10):length(p)]);
end

scatter(gamma_range, periodicities./1000 , legend=false)
